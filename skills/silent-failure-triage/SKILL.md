...
- **验证于**：Windows 11 家庭中文版 10.0.26200 · Git Bash 5.2.37 / cmd / Windows PowerShell 5.1.26100.9444 / Node v24.18.0 · 2026-09-17
  （实测出入：`$?` 说谎的是 **bash 与 cmd**：bash 管道后 `$?`=0 而 `${PIPESTATUS[0]}`=3；cmd 管道后 `%ERRORLEVEL%`=1（末命令）而直跑=3；PowerShell 5.1 的 `$LASTEXITCODE` 管道后仍报 3，本机未复现其"永远 0"。）
