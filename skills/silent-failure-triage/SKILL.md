...
## 13. 校验脚本自己会失效：对照组没进扫描面、缺失文件被读成"内容不同"

- **现象**：同一轮收口里撞到的两支，方向相反、同源。
...
- **根因**：校验的三要素——**扫描面**、**对照组**、**缺失值的表示**——任一坏掉，输出都和"真结论"长得一模一样。`git grep` 对未跟踪文件沉默；shell 把工具的失败折叠成空串；空串参与 `=` 比较时既不报错也不特殊化。反过来这支也存在：出门内容里若混进一个**当时还没被跟踪**的文件，扫描面就漏了它，那 0 命中是假干净。
...
- **判定**：对任何 0 命中或全量命中，先回答一句——"这个数字是在**真的扫描面/比对集**上得到的，还是在工具悄悄跳过的那部分上得到的？" 可执行的证法：同一轮里放两个对照样本，一个**已知在扫描面内**、一个**已知在扫描面外**（未跟踪文件即天然如此），只有前者命中、后者不命中，才算"过滤器有效 **且** 扫描面已证"；只测前者会漏掉 ①，只测后者会误判成扫描器坏了。
- **验证于**：Windows 11 家庭中文版 10.0.26200 · Git Bash 5.2.37（MSYS 3.6.6 / git 2.53.0.windows.2）· Python 3.12.10 · 2026-09-18
  （实测：`git grep -F <清单词>` 对一个含该词的**未跟踪**临时文件报 0 命中；同一文件 `git add -N` 后立即命中 1，`git reset -- <该文件>` 并删除后 `git status --porcelain` 恢复为空。②：`md5sum <不存在路径> | awk '{print $1}'` 得空串，与真实哈希比较判为不等 ⇒ 10 个技能全报 MISMATCH；纠正路径层数并加 `[ ! -f ] → ABSENT` 分支后重跑，同一批 10 项一致、缺失 0 项。）
