...
- **复测确认（2026-09-19，另一 agent 独立复跑）**：文件侧 **259 成功 / 260 抛 `FileNotFoundError` errno=2，与本节逐字一致**。目录侧它最初在 247 / 248 看到 `list OK`，并提出「是 base 路径长度不同造成的偏移」——**该假设被跨 base 复测推翻**：把 base 取 53 与 75 两个长度后 `os.mkdir` 的断点**都是 247 OK / 248 FAIL**，阈值只由**完整路径长度**决定、与 base 无关；它没复现的真因是**调用通道不同**（Node 走 libuv 自动补前缀，见根因那条）。同批还确认了失败文案的两档分界（248–259 `文件名或扩展名太长` vs 260 起 `系统找不到指定的路径`）。教训：本条的数字必须连同**测量通道**一起引用，否则下一个复核者必然对不上。
