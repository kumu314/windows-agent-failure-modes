...
- **判据收紧之二（2026-09-21，同一颗技能的另一例；这次不报错）**：上一条至少抛 `SyntaxError`，本轮还有一个**静默**版本，代价更大。用 `cat > 探测脚本.py <<'PY'` 落盘时，门禁正则里的字符类 `[\\/]`（反斜杠两次 + 正斜杠）在磁盘上变成 `[/]` —— 只剩正斜杠。脚本语法完好、照跑，只是**不再匹配任何反斜杠形态**：同一批文件，Write 工具落盘的脚本命中 13，heredoc 落盘的命中 6。两个“看起来一模一样”的正则给出两遍读数 ⇒ 我一度把它读成“导出树自身不一致”，追了约 8 轮往返，还错猜过一次“公开文本里泄漏了路径”（复看原文件即否掉）。
  ⇒ 三条推论：① 探测/门禁脚本一律用 Write 工具落盘，**不要因为“它只是脚本”就放行 heredoc**——被改坏的是**测量仪器**，比被测量对象更贵；② 两通道读数不一致时，先逐字节比**脚本文件本身**（`md5sum`，或 `python -c` 配 `repr(open(...).read()[i:j])` 看那一段字面），再怀疑被测对象；③ 校验“两个正则相同”不能靠肉眼或 `diff` 前的印象，要比磁盘字节。
...
## 19. Git Bash 把参数位"像路径"的串改写成文件系统路径：判别式是**前导 `/` 且不含 `?`**，与 `=`、与路径是否存在都无关

- **现象（在 gh 身上撞到的）**：`gh api /repos/<o>/<r>/git/ref/heads/main` 不返回 404、也不说没权限，而是报 `invalid API endpoint: "C:/Program Files/Git/repos/<o>/<r>/git/ref/heads/main". Your shell might be rewriting URL paths as filesystem paths. To avoid this, omit the leading slash from the endpoint argument`；同一条命令**去掉前导斜杠**（`gh api repos/<o>/<r>/git/ref/heads/main`）当场返回 sha。bash 侧**没有任何异常痕迹**——改写发生在"传给原生 exe 的那一跳"，不报错、不改退出码。
- **判别式（11 例，同一 shell、同一 argv 回显探针，逐格比对送入/收到的 `argv[1]`）**：
  - **仍被改写**：`/repos/o/r/git/ref/heads/main` → 前缀 `C:/Program Files/Git/…`；**单层** `/repos`；**带 `=`** 的 `/repos/o/r/topics/x=y`；**反斜杠形** `/repos\o\r`（斜杠先归一再前缀）；**带空格** `/repos/o/r/a b`；**路径根本不存在**的 `/nonexistent-zz-<日期>/y`——同一串在 bash 侧用 `[ -e ]` 答"不存在"，而原生进程收到的是 `C:/Program Files/Git/…` ⇒ 改写与存在性无关，也不是"真去找了这个目录"；真实 POSIX 路径 `/d/<…>/temp` → `D:/<…>/temp`、`/c/Windows/System32/where.exe` → `C:/Windows/System32/where.exe`。
  - **不被改写**：**整串含 `?`**（`/repos/o/r/branches?per_page=100`、`/repos/o/r/issues?frag`，有无 `=` 都一样）；**无前导斜杠**（`repos/o/r/git/ref/heads/main` 原样送到）。
  - **与 §7 衔接的一格**：`//F` → 原生进程收到 `/F`（这正是 §7 里 `taskkill //F` 能生效的机制）。
  - ⇒ **唯一稳定的判据 = 前导 `/` 且整串不含 `?`**。**本轮先假设"`?` 或 `=` 有一个就安全"，被 `topics/x=y` 那一例当场证伪**：`=` 完全不参与，`?` 才是开关。
- **两条"确实能挡住"的全局修法各带一次反噬**：`MSYS2_ARG_CONV_EXCL='*'` 与 `MSYS_NO_PATHCONV=1` 都让伪路径原样送到，但**同时挡住真路径的转换**——同一探针下 `os.path.exists('/d/…/<探针脚本名>')` 由 **True 变 False**，原生 python 报 `can't open file` 并把没转换的 `/d/…` 拼到当前盘符下，**连自己的脚本都打不开**。⇒ 全局关改写会把 §4/§5 那条"MSYS 路径只有 bash 认"反向放大成"连原生程序的常规调用都坏"。**限定模式 `MSYS2_ARG_CONV_EXCL='/repos/*'` 在本机完全无效**（伪路径照样被改写；根因未查，只记形状）。
- **对策（按代价排序）**：① **首选去掉前导斜杠**（gh 自己也是这么提示的；只改参数形态，不动任何其他行为）；② 别指望引号：实测探针里的参数**一直是双引号包住的**，照样被改写 ⇒ 转换发生在引号剥离之后；③ 全局开关只在"这一整条命令里没有任何真路径参数"时用，且**先自证无反噬**——同一条命令里带一次 `python -c "import sys,os;print(os.path.exists(sys.argv[1]))" <脚本路径>`，看到 True 才继续。
- **探针自身的两条坑（本轮踩到，否则读数会假）**：① 第一版把**被测脚本自己的路径也写成 `/d/…` 绝对形态**，于是 `EXCL='*'` 那两格测到的是"python 打不开脚本"而不是"改写有没有发生"，两格作废重跑；改成先 `cd` 进目录、用**相对脚本名**调用，才做到"只有被测参数是绝对形态"。② 判"改没改"要**逐格比对送/收字符串**（`[ "$got" = "$sent" ]`），不要肉眼看斜杠——反斜杠归一那一格肉眼极易漏。
- **根因（本轮量到的是"哪一跳"，不是"为什么含 `?` 就放行"）**：改写不在 bash 的参数展开阶段，而在 **bash 把 argv 交给目标进程的那一跳，且只对原生 exe 生效**。同一串 `/nonexistent-zz-<日期>/y` 的逐跳读数（第四轮探针）：bash 侧变量展开与 `echo` 回显**原样**、`[ -e ]` 答"不存在"、`echo` 的 rc=0；原生 Windows 版 python 的 `argv[1]` **已是前缀成安装目录形态的改写值**（见上面各格回显）、rc 仍为 0；msys 重链接的 `/usr/bin/env` 在报错正文里回显的是**原样**（rc=127）。⇒ 三条含义：① 转换**不查文件系统**，不存在的伪路径照样被前缀；② 这一跳**不报错、不改退出码**，所以"没报错"根本不是"没被改写"的证据；③ 只有目标是原生 exe 时才中招，msys 自带工具不受影响。至于"含 `?` 就放行"的底层判据，本轮只量到"是什么"，未读 MSYS2 argconv 的源码与文档，不写成已知机制。
- **判定**：一条参数位字符串会不会被 MSYS 改写，只看两件事——**是否以 `/` 起头**、**整串是否含 `?`**；其余因素（有无 `=`、路径在不在、有无空格、正斜杠还是反斜杠）都不改变结果。⇒ 原生程序收到 `C:/Program Files/Git/…` 这种值时，先按"前导斜杠被 bash 改写"处理，不要去查权限、也不要先去确认路径存不存在。
- **状态**：**已实测**（11 例判别式 + 四类接收方（bash 展开 / 原生 python / msys 重链接 env / 原生 gh）的逐跳对照 + 两种全局修法的副作用 + 限定模式无效 + gh 前后对照）。**未测**：其他 MSYS2/Cygwin 版本的改写规则与官方文档口径、除 gh 之外其他转发 CLI 的受害面；`MSYS2_ARG_CONV_EXCL` 限定模式无效的根因未查。
- **验证于**：Windows 11 家庭中文版 10.0.26200.9457（zh-CN）· Git Bash 5.2.37(1)-release(x86_64-pc-msys) · 原生 Windows 版 Python 3.12.10 与 msys 重链接的 `/usr/bin/env`（两者都只做 argv 回显，无副作用）· gh 2.97.0 · 2026-09-23
  （四轮 temp 内探针，读数文件都在本机一次性 temp 目录里、不随包发布：第一轮 8 例（其中两格因脚本自身路径被改写而作废）、第二轮修法对照 + `os.path.exists` 的 `True / False / False` 三行副作用、第三轮 11 例判别式收口（含"不存在仍改""`//F` → `/F`""`/c/Windows/…` → `C:/Windows/…`"）、第四轮逐跳对照（bash 原样 rc=0 / 原生 python 改写 rc=0 / msys 重链接 env 原样 rc=127）。第四轮里 `where.exe` 那一格因它自己的参数语法先报错、正文又是 gbk 乱码，**读数作废未采用**。gh 那条对照是对本包仓库的一次只读 `gh api`，除此之外无写操作、无对真实系统的改动。）
