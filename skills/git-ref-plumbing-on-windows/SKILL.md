...
- **状态**：**读侧与写侧均已实测**。读侧 = 分页形状 + `head=` 查询的命中/空集两种返回（2026-09-22 戳）；写侧 = 两条删除通道（REST `DELETE …/git/refs/heads/<b>` 与 `git push --delete`）+ 删后终态判据，在一次性靶场仓上按维护者授权执行（2026-09-23 戳）。**下一回合补上**：那条本轮未删的 PR 分支也按同一判据删掉了（先 `head=` 正控证它合并过才删），终态 `GET /branches` 只剩 `main`、257 B，与 `git ls-remote --heads` 同刻同份；读数见 silent-failure-triage §7 的 2026-09-24 验证于。**仍未量**：多分支批量删（本轮两分支各单发、一分支一条通道）、删错后的恢复路径（未做，也不建议拿真仓试）。
