...
| Skill | 一句话 |
|---|---|
| [shell-quoting-and-path-forms](skills/shell-quoting-and-path-forms/SKILL.md) | Git Bash/MSYS 的引号与路径形态：命令替换吃掉反引号、heredoc 吞反斜杠、`/d/` 与 `/tmp` 原生程序不认、`//F` 被重写后静默失败、含空格路径被拆成多参数、`$(…)` 剥掉尾换行造成哈希假不等、过 260 字符后半数工具连"文件存在"都看不见、junction 各家答案不同、`[ -L ]` 取决于有没有尾斜杠，而吃掉目标内容的是路径尾反斜杠和 `del /f /s /q`；另有 PowerShell 5.1 没有 `if` 表达式（报错却指向 `if`，像整条命令坏了）、长正文别当参数传（`git commit -F 文件`） |
| [silent-failure-triage](skills/silent-failure-triage/SKILL.md) | "退出码 0 但没做成"的总账：管道让 `$?` 说谎（bash 与 cmd，PowerShell 不受影响）、空输出的三种成因（含下游工具没装时字段变空串而退出码仍是 0）、假同步与假陈旧、计划任务注册了却永不运行、解释器"存在"是 0 字节壳、校验脚本自己会失效（对照样本没进扫描面所以对照不响；缺失文件的哈希是空串，被读成"内容不同"而全量报错；写成多行的检查链里某步静默失败而总结论照样打印）、"命中 N 处"没有分母就不是证据（19 与 77 可以同时都对，两套口径报出同一个数则是去重静默失效）、守卫本身从没生效过（netstat 列序理解反了⇒条件恒假；`catch` 返回同形哨兵⇒条件恒真）、200 与"端口没监听"可以同时都是假的、识别类工具返回成功而内容是编的、渲染层静默失效五支（样式覆盖内联属性 / 主题变量反解 / 布局差一列 / 渲染入口报错塌成空白面板 / 派生数量对不上） |
...
| [stale-output-layers](skills/stale-output-layers/SKILL.md) | "改了没生效"的分层定位：源码 / 构建 / 服务 / 渲染 / 部署五层缓存，再加一层数据快照（只拷主库文件会静默读到旧事务），每层一条独立判据；Service Worker 清理的两个顺序约束、备份放项目内会弄坏构建、收尾要一次真实渲染证据 |

## 结构约定

- 每条固定四段：**现象 → 根因 → 对策 → 判定**。判定是可执行命令，不是形容词。
- `description` 有两条硬约束——客户端只把它的前一段喂给模型（本机实测约 276 字符），而它的 YAML 标量写法一错就让整颗技能**静默不加载**（不报错、CI 也不响）。判据与正确写法见 `CONTRIBUTING.md`「`description` 的两条硬约束」；三种坏形状现在由 lint 按 error 拦。
